...
/**
 * Busca la transacción de firma más reciente con un hash de documento dado.
 * Útil para que /api/documentos recupere la fecha/hora de una firma ya
 * registrada al validarla.
 *
 * @param {string} hashDocumento - hash SHA-256 (hex) del XML firmado.
 * @returns {Promise<object|null>}
 */
async function buscarTransaccionFirmaPorHash(hashDocumento) {
  const { rows } = await pool.query(
    `
    SELECT id_transaccion_firma, id_contribuyente, serial_certificado, hash_documento, resultado, fecha_hora
    FROM transacciones_firma
