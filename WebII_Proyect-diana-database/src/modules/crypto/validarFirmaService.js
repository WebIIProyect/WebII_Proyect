...
/**
 * Extrae la llave pública embebida en el primer <Signature> de un XML ya
 * firmado, sin validar la firma. Útil para ubicar qué certificado emitió
 * un documento antes de decidir contra qué llave validarlo "fuerte".
 *
 * @param {string} xmlFirmado
 * @returns {string|null} PEM de la llave pública, o null si no se encontró.
 */
function extraerLlavePublicaDeXMLFirmado(xmlFirmado) {
  if (!xmlFirmado || typeof xmlFirmado !== 'string') return null;

  let documento;
  try {
    documento = new DOMParser().parseFromString(xmlFirmado, 'text/xml');
  } catch {
    return null;
  }

  const nodosFirma = documento.getElementsByTagNameNS(NAMESPACE_XMLDSIG, 'Signature');
  if (!nodosFirma || nodosFirma.length === 0) return null;

  return extraerLlavePublicaDeKeyInfo(nodosFirma[0]);
}

module.exports = {
  validarFirmaXML,
  extraerLlavePublicaDeKeyInfo,
  extraerLlavePublicaDeXMLFirmado,
};
